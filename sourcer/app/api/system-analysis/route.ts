...
export async function GET(req: NextRequest) {
  try {
    ...
    const incomeStatement = (incomeStatementRows as any)[0];
    ...
    const balanceSheet = (balanceSheetRows as any)[0];
    ...
    const studentResults = (studentResultsRows as any).map((row: any) => ({
      id: row.id,
