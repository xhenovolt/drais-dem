...
export async function POST(req: NextRequest) {
  ...
  await conn.execute('INSERT INTO classes (school_id,name,class_level,head_teacher_id,curriculum_id) VALUES (?,?,?,?,?)', [body.school_id || 1, body.name, body.class_level || null, body.head_teacher_id || null, body.curriculum_id || null]);
...
export async function PUT(req: NextRequest) {
  ...
  const conn = await getConnection();
  await conn.execute('UPDATE classes SET name=?, class_level=?, head_teacher_id=?, curriculum_id=?, school_id=? WHERE id=?', [body.name, body.class_level || null, body.head_teacher_id || null, body.curriculum_id || null, body.school_id || 1, body.id]);
  await conn.end();
...
export async function DELETE(req: NextRequest) {
  ...
  const conn = await getConnection();
  await conn.execute('DELETE FROM classes WHERE id=?', [body.id]);
  await conn.end();
  return NextResponse.json({ success: true });
}
