...
export async function GET(req: NextRequest){
  ...
  const [rows] = await conn.execute(`SELECT l.id,l.wallet_id,w.name wallet_name,l.category_id,l.tx_type,l.amount,l.reference,l.description,l.student_id,l.staff_id,l.created_at FROM ledger l JOIN wallets w ON w.id=l.wallet_id ${whereSql} ORDER BY l.id DESC LIMIT ? OFFSET ?`,[...params,per_page,offset]);
...
export async function POST(req: NextRequest){
  ...
  try {
    ...
    `, [1, 1, (result as any).insertId, JSON.stringify({ amount, tx_type, reference })]);
