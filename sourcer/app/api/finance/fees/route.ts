...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const enhancedItems = FinanceService.enhanceFeeItems(feeItems as any);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (template_id) {
      ...
      if (!(template as any).length) {
        throw new Error('Fee template not found');
      }

      const templateItems = JSON.parse((template as any)[0].items);
      ...
      return NextResponse.json({
        success: true,
        message: `Fee template applied to ${(students as any).length} students`
      });
      ...
      for (const item of items) {
        ...
        // Update status
        await FinanceService.updateFeeItemStatus((result as any).insertId, connection);
      }
