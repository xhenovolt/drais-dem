  ...
  try {
    ...
    return NextResponse.json({
      ...
      pagination: {
        ...
        limit,
        offset,
        hasMore: offset + limit < (countResult as any)[0].total
      }
    });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if ((feeItems as any[]).length !== items.length) {
      ...
      const receiptNo = `R-${new Date().getFullYear()}-${String((receiptCount as any)[0].count + 1).padStart(6, '0')}`;
      ...
      `, [
        school_id, wallet_id, (incomeCategory as any)[0]?.id || 1, amount,
        reference || receiptNo, `Payment from ${paid_by} for ${(feeItems as any[]).map((f: any) => f.item).join(', ')}`,
      ...
      `, [
        ...
        JSON.stringify({
          items: (feeItems as any[]).map((f: any) => ({ item: f.item, amount: f.amount })),
