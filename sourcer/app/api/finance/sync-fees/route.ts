...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session?.user || !(session.user as any)?.school_id) {
      return new NextResponse('Unauthorized', { status: 401 });
    }

    const school_id = (session.user as any).school_id;
    ...
    await withTransaction(async (conn) => {
      for (const student of (missingStudents as any[])) {
        ...
        for (const fee of feesToInsert) {
          ...
          `, [student.student_id, student.term_id, (fee as any).item, (fee as any).amount]);
        }
      }
    });
    ...
    return NextResponse.json({
      success: true,
      message: `Synchronized fees for ${(missingStudents as any[]).length} students`
    });
