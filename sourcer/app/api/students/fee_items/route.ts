...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (!(students as any).length || !(feeItems as any).length) {
      ...
      return NextResponse.json({ success: false, message: 'Failed to add fee items.', error: (error as any).message }, { status: 500 });
    } finally {
