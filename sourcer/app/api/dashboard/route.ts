...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      data: {
        total_classes: (classes as any)[0]?.total_classes || 0,
        total_learners: (learners as any)[0]?.total_learners || 0,
        boys: (genderCounts as any)[0]?.boys || 0,
        girls: (genderCounts as any)[0]?.girls || 0,
        total_staff: (staff as any)[0]?.total_staff || 0,
        total_parents: (parents as any)[0]?.total_parents || 0,
        admissions: (admissions as any)[0] || {},
        payment_stats: (paymentStats as any)[0] || {},
        learner_performance: (learnerPerformance as any)[0] || {},
        best_learner: (bestLearner as any)[0] || null,
        worst_learner: (worstLearner as any)[0] || null,
        term_progress: (termProgress as any)[0] || {
