...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      // AI insights placeholder - would require AI insights table
      Promise.resolve([])
    ]);
    ...
    const totalStudentsToday = (attendanceData as any)?.total_students || 0;
    const presentToday = (attendanceData as any)?.present_count || 0;
    const attendancePercentage = totalStudentsToday > 0 ? Math.round((presentToday / totalStudentsToday) * 100) : 0;

    const overview = {
      kpis: {
        totalStudents: (studentCount as any)?.total_learners || 0,
        presentToday: presentToday,
        absentToday: (attendanceData as any)?.absent_count || 0,
        attendancePercentage: attendancePercentage,
        enrollmentGrowth: (admissionData as any)?.last_month || 0,
        feesCollectedToday: 0, // Will be calculated from recent payments
        defaultersCount: ((paymentData as any)?.not_paid || 0) + ((paymentData as any)?.partially_paid || 0)
      },
      schoolStats: {
        total_classes: (classCount as any)?.total_classes || 0,
        total_learners: (studentCount as any)?.total_learners || 0,
        boys: (genderData as any)?.boys || 0,
        girls: (genderData as any)?.girls || 0,
        total_staff: (staffCount as any)?.total_staff || 0,
        total_parents: (parentCount as any)?.total_parents || 0
      },
      admissions: (admissionData as any) || {},
      paymentStats: {
        fully_paid: (paymentData as any)?.fully_paid || 0,
        partially_paid: (paymentData as any)?.partially_paid || 0,
        not_paid: (paymentData as any)?.not_paid || 0,
        total_outstanding: (paymentData as any)?.total_outstanding || 0
      },
      performance: {
        improving_learners: (performanceData as any)?.improving_learners || 0,
        declining_learners: (performanceData as any)?.declining_learners || 0,
        average_score: (performanceData as any)?.average_score || 0,
        students_with_results: (performanceData as any)?.students_with_results || 0
      },
      topPerformers: bestStudentData ? [{
        id: 1,
        name: `${(bestStudentData as any).first_name} ${(bestStudentData as any).last_name}`,
        className: 'N/A',
        average: (bestStudentData as any).best_score,
        rank: 1,
        trend: 'improving',
        photoUrl: null
      }] : [],
      worstPerformers: worstStudentData ? [{
        id: 2,
        name: `${(worstStudentData as any).first_name} ${(worstStudentData as any).last_name}`,
        className: 'N/A',
        average: (worstStudentData as any).worst_score,
        rank: 1,
        isAtRisk: true,
        photoUrl: null
      }] : [],
      fees: {
        totalExpected: ((paymentData as any)?.total_outstanding || 0) + 1000000, // Mock expected amount
