...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const studentId = (fingerprintResult as any[])[0].student_id;
    ...
    const classId = Array.isArray(classResult) && classResult.length > 0 
      ? (classResult as any[])[0].class_id : null;
    ...
    const studentName = Array.isArray(studentResult) && studentResult.length > 0
      ? `${(studentResult as any[])[0].first_name} ${(studentResult as any[])[0].last_name}`
      : 'Unknown Student';
