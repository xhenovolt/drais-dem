...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (search) {
      ...
      queryParams.push(`%${search}%` as any, `%${search}%` as any, `%${search}%` as any);
    }
    ...
    if (department) {
      ...
      queryParams.push(parseInt(department) as any);
    }
    ...
    const total = Array.isArray(countRows) ? (countRows[0] as any)?.total || 0 : 0;
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({
        ...
        data: {
          id: (staffResult as any).insertId,
