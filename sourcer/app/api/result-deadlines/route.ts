...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const deadline of deadlines) {
      ...
      if ((teacher as any).length > 0 && (teacher as any)[0].phone) {
        const phone = (teacher as any)[0].phone;
        ...
        try {
          ...
          console.error(`Failed to send SMS to ${phone}:`, (error as any)?.message || error);
        }
      }
    }
