...
const TimetablePage = () => {
    ...
    try {
      const response = await fetch('/api/timetable/conflicts', {
        ...
        body: JSON.stringify({
          ...
          recurrence: lesson.recurrence_pattern || 'none'
        })
      });
  ...
  const handleCreateException = (lesson: Lesson) => {
    ...
    toast('Exception management feature coming soon!', { icon: 'ℹ️' });
  };
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      {lessonConflicts.length > 0 && (
                        <ConflictIndicator 
                          ...
                          conflictDetails={`${lessonConflicts.length} conflicts found`}
                        />
                      )}
                      ...
                      <RecurringIndicator 
                        ...
                        pattern={lesson.recurrence_pattern}
                      />
                    ...
                    {lessonConflicts.length > 0 && (
                      <div className="mt-3">
                        <ConflictIndicator 
                          hasConflict={true}
                          conflictType="teacher"
                          conflictDetails={lessonConflicts.map(c => c.message).join(', ')}
                        />
                      </div>
                    )}
                    ...
                    {lesson.exception_reason && (
                      <div className="mt-2 p-2 bg-yellow-50 dark:bg-yellow-500/10 border-l-4 border-yellow-400 dark:border-yellow-500/50 rounded">
                        ...
                        {lesson.new_date && lesson.new_start_time && lesson.new_end_time && (
              ...
              <div className="flex flex-wrap gap-1">
                {lessonConflicts.length > 0 && (
                  <ConflictIndicator 
                    hasConflict={true}
                    conflictType="teacher" 
                    conflictDetails={`${lessonConflicts.length} conflicts`}
                  />
                )}
                <RecurringIndicator 
                  isRecurring={lesson.is_recurring}
