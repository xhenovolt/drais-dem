...
export default function TheologyResultsManager() {
  ...
  // Fetch theology classes - ensure it's always an array
  const { data: theologyClassesData } = useSWR<{ data?: TheologyClass[] } | TheologyClass[]>(
    '/api/classes?type=theology',
    fetcher
  );

  // Safely extract theology classes array
  const theologyClasses: TheologyClass[] = Array.isArray(theologyClassesData) 
    ? theologyClassesData 
    : theologyClassesData?.data || [];

  // Fetch terms - ensure it's always an array
  const { data: termsData } = useSWR('/api/terms', fetcher);
  const terms = Array.isArray(termsData) ? termsData : termsData?.data || [];

  // Fetch theology subjects - ensure it's always an array
  const { data: theologySubjectsData } = useSWR(
    '/api/subjects?type=theology',
    fetcher
  );
  const theologySubjects = Array.isArray(theologySubjectsData) 
    ? theologySubjectsData 
    : theologySubjectsData?.data || [];
              ...
              <div className="p-6 overflow-y-auto max-h-[60vh]">
                ...
                <div className="space-y-3">
                  <h4 className="font-semibold text-slate-800 dark:text-white">
                    Students in {theologyClasses.find(c => c.id.toString() === selectedTheologyClass)?.name || 'Selected Class'}
                  </h4>
